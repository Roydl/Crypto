namespace Roydl.Crypto.Test.ChecksumTests
{
    ...
    {
        private const ChecksumAlgo Algorithm = ChecksumAlgo.Crc40;
        private const int HashBits = 40;
        private const int HashSize = 10;
        private const int RawHashSize = 5;
        private const string ExpectedTestHash = "10df47b471";
        private const string ExpectedRangeHash = "f46d283fad";
        ...
        private static Crc40 _instanceDefault, _instanceStream, _instanceByteArray, _instanceString, _instanceFilePath;

        [OneTimeSetUp]
        public void CreateInstances()
        {
            _instanceDefault = new Crc40();
            using (var ms = new MemoryStream(TestVars.TestBytes))
                _instanceStream = new Crc40(ms);
            _instanceByteArray = new Crc40(TestVars.TestBytes);
            _instanceString = new Crc40(TestVars.TestStr);
            File.WriteAllBytes(TestFilePath, TestVars.TestBytes);
            _instanceFilePath = new Crc40(TestFilePath, true);
        }
        ...
        {
            var instanceDefault = new Crc40();
            Assert.IsInstanceOf(typeof(Crc40), instanceDefault);
            ...
            Crc40 instanceStream;
            using (var ms = new MemoryStream(TestVars.TestBytes))
                instanceStream = new Crc40(ms);
            Assert.IsInstanceOf(typeof(Crc40), instanceStream);
            ...
            var instanceByteArray = new Crc40(TestVars.TestBytes);
            Assert.IsInstanceOf(typeof(Crc40), instanceByteArray);
            ...
            var instanceString = new Crc40(TestVars.TestStr);
            Assert.IsInstanceOf(typeof(Crc40), instanceString);
            ...
            var instanceFilePath = new Crc40(TestFilePath, true);
            Assert.IsInstanceOf(typeof(Crc40), instanceFilePath);
            ...
            var instanceFileInfo = new Crc40(new FileInfo(TestFilePath));
            Assert.IsInstanceOf(typeof(Crc40), instanceFileInfo);
        ...
        {
            Assert.AreEqual(_instanceDefault.GetHashCode(), new Crc40().GetHashCode());
        ...
        public void InstanceOperators()
        {
            ...
            Assert.AreEqual(_instanceStream.HashNumber, (ulong)_instanceByteArray);
