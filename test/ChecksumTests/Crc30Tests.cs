namespace Roydl.Crypto.Test.ChecksumTests
{
    ...
    {
        private const ChecksumAlgo Algorithm = ChecksumAlgo.Crc30;
        private const int HashBits = 30;
        private const int HashSize = 8;
        private const int RawHashSize = 4;
        private const string ExpectedTestHash = "09fc8f98";
        private const string ExpectedRangeHash = "a74e1078";
        ...
        private static Crc30 _instanceDefault, _instanceStream, _instanceByteArray, _instanceString, _instanceFilePath;

        [OneTimeSetUp]
        public void CreateInstances()
        {
            _instanceDefault = new Crc30();
            using (var ms = new MemoryStream(TestVars.TestBytes))
                _instanceStream = new Crc30(ms);
            _instanceByteArray = new Crc30(TestVars.TestBytes);
            _instanceString = new Crc30(TestVars.TestStr);
            File.WriteAllBytes(TestFilePath, TestVars.TestBytes);
            _instanceFilePath = new Crc30(TestFilePath, true);
        }
        ...
        {
            var instanceDefault = new Crc30();
            Assert.IsInstanceOf(typeof(Crc30), instanceDefault);
            ...
            Crc30 instanceStream;
            using (var ms = new MemoryStream(TestVars.TestBytes))
                instanceStream = new Crc30(ms);
            Assert.IsInstanceOf(typeof(Crc30), instanceStream);
            ...
            var instanceByteArray = new Crc30(TestVars.TestBytes);
            Assert.IsInstanceOf(typeof(Crc30), instanceByteArray);
            ...
            var instanceString = new Crc30(TestVars.TestStr);
            Assert.IsInstanceOf(typeof(Crc30), instanceString);
            ...
            var instanceFilePath = new Crc30(TestFilePath, true);
            Assert.IsInstanceOf(typeof(Crc30), instanceFilePath);
            ...
            var instanceFileInfo = new Crc30(new FileInfo(TestFilePath));
            Assert.IsInstanceOf(typeof(Crc30), instanceFileInfo);
        ...
        {
            Assert.AreEqual(_instanceDefault.GetHashCode(), new Crc30().GetHashCode());
