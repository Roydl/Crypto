namespace Roydl.Crypto.Test.ChecksumTests
{
    ...
    public class Crc21Tests
    {
        private const ChecksumAlgo Algorithm = ChecksumAlgo.Crc21;
        private const int HashBits = 21;
        private const int HashSize = 6;
        private const int RawHashSize = 3;
        private const string ExpectedTestHash = "d28a0d";
        private const string ExpectedRangeHash = "d6c0c0";
        ...
        private static Crc21 _instanceDefault, _instanceStream, _instanceByteArray, _instanceString, _instanceFilePath;

        [OneTimeSetUp]
        public void CreateInstances()
        {
            _instanceDefault = new Crc21();
            using (var ms = new MemoryStream(TestVars.TestBytes))
                _instanceStream = new Crc21(ms);
            _instanceByteArray = new Crc21(TestVars.TestBytes);
            _instanceString = new Crc21(TestVars.TestStr);
            File.WriteAllBytes(TestFilePath, TestVars.TestBytes);
            _instanceFilePath = new Crc21(TestFilePath, true);
        }
        ...
        {
            var instanceDefault = new Crc21();
            Assert.IsInstanceOf(typeof(Crc21), instanceDefault);
            ...
            Crc21 instanceStream;
            using (var ms = new MemoryStream(TestVars.TestBytes))
                instanceStream = new Crc21(ms);
            Assert.IsInstanceOf(typeof(Crc21), instanceStream);
            ...
            var instanceByteArray = new Crc21(TestVars.TestBytes);
            Assert.IsInstanceOf(typeof(Crc21), instanceByteArray);
            ...
            var instanceString = new Crc21(TestVars.TestStr);
            Assert.IsInstanceOf(typeof(Crc21), instanceString);
            ...
            var instanceFilePath = new Crc21(TestFilePath, true);
            Assert.IsInstanceOf(typeof(Crc21), instanceFilePath);
            ...
            var instanceFileInfo = new Crc21(new FileInfo(TestFilePath));
            Assert.IsInstanceOf(typeof(Crc21), instanceFileInfo);
        ...
        {
            Assert.AreEqual(_instanceDefault.GetHashCode(), new Crc21().GetHashCode());
